...
export default function BlogDetails() {
  ...
  const action = (
    <React.Fragment>
      <IconButton size="small" aria-label="close" color="inherit" onClick={likeblogclose}>
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12} md={8}>
        <Button onClick={editblog} disabled={buttonDisabled}>
          Edit Blog
        </Button>
        <Button onClick={confirmdelete} disabled={buttonDisabled}>
          Delete Blog
        </Button>
      </Grid>
      <Grid item xs={12} md={8}>
      ...
      <Grid item xs={12} md={4}>
        ...
        <form onSubmit={addComment}>
          ...
          />
          <Button type="submit" variant="contained" sx={{ marginBottom: 2, bgcolor: '#2E8BC0' }}>
            Submit
          </Button>
        </form>
        ...
        {commentsData?.map(
          (comments2: { userID: any; commentID: any; comment: any; first_name: any }) => {
            return (
              <CommentCard
